...
# Move DEAP creator definitions to module level
...
def calculate_total_distance(individual, locations):
    ...
    distance += haversine(loc1['lat'], lon1, loc2['lat'], loc2['lng']) # FIXED: lon1 was undefined, should be loc1['lng']
    return distance,

# Fixed version of calculate_total_distance
def calculate_total_distance_fixed(individual, locations):
    distance = 0
    for i in range(len(individual) - 1):
        loc1 = locations[individual[i]]
        loc2 = locations[individual[i+1]]
...
def solve_tsp(locations):
    ...
    toolbox.register("evaluate", calculate_total_distance_fixed, locations=locations)
    ...
    total_distance = best_indices.fitness.values[0]
