...
# Initialize DEAP creator at module level to avoid re-definition errors
...
def solve_tsp(locations):
    ...
    stats.register("avg", lambda x: math.fsum(x) / len(x))
    stats.register("min", min)

    algorithms.eaSimple(pop, toolbox, cxpb=0.7, mutpb=0.2, ngen=100, stats=stats, halloffame=hof, verbose=False)
