...
def optimize_route():
    ...
    for loc in locations:
        if not isinstance(loc, dict) or 'lat' not in loc or 'lng' not in loc or \
           not isinstance(loc['lat'], (int, float)) or not isinstance(loc['lng'], (int, float)):
    ...
    if len(locations) < 2:
        return jsonify({
            ...
            "status": "success",
            "message": "Route optimized successfully (single location)"
        })

    toolbox = create_toolbox(locations)
