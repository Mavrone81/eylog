  ...
  Mutation: {
    createDelivery: async (_, { customerId, origin, destination }) => {
      ...
      try {
        ...
        if (populatedDelivery.customer && populatedDelivery.customer.phone) {
          await sms.sendSMS(populatedDelivery.customer.phone, `Your delivery to ${destination.address} has been created.`);
        }
    ...
    assignDriver: async (_, { deliveryId, driverId }) => {
      ...
      try {
        ...
        if (delivery.customer && delivery.customer.phone) {
          await sms.sendSMS(delivery.customer.phone, `Driver ${driver.name} has been assigned to your delivery.`);
        }
    ...
    updateDeliveryStatus: async (_, { id, status }) => {
      ...
      try {
        ...
        if (driverToUpdate) {
          await redisClient.set(`driver:${driverToUpdate}:status`, 'AVAILABLE');
        }

        if (delivery.customer && delivery.customer.phone) {
          await sms.sendSMS(delivery.customer.phone, `Your delivery status has been updated to: ${status}`);
        }
