...
const resolvers = {
  ...
  Mutation: {
    createDelivery: async (_, { origin, destination, customerId }) => {
      const delivery = new Delivery({ origin, destination, customerId, updatedAt: new Date() });
    ...
    assignDriver: async (_, { deliveryId, driverId }, { redisClient, pool }) => {
      ...
      await pool.query('INSERT INTO delivery_events (delivery_id, event_type, payload) VALUES ($1, $2, $3)',
